describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for(var i = 0; i<prodItems.length; i+=1) {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let innerTextValue = innerTextProperty['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$("product-item");
    for(var i = 1; i<prodItems.length; i+=1) {
      const shadowRoot = await prodItems[i].getProperty("shadowRoot");
      const b = await shadowRoot.$("button");
      await b.click();
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i<prodItems.length; i+=1) {
      const shadowRoot = await prodItems[i].getProperty("shadowRoot");
      const b = await shadowRoot.$("button");
      let innerTextProperty = await b.getProperty('innerText');
      let innerTextValue = innerTextProperty['_remoteObject'].value;
      expect(innerTextValue).toBe('Remove from Cart');
    }
    const cart = await page.$("#cart-count");
